...
{
    public class HubAllowedRule : VerificationRuleBase<PushSiteColumnAction, PushContentTypeAction>, IPushVerificationRule
    {
        ...
        public override VerificationRuleResult Verify(PushSiteColumnAction action)
        {
            using (Logger.MethodTraceLogger(action.SiteColumn.Title, action.TargetSiteCollection))
            {
                try
                {
                    Console.WriteLine(@"Verify hub allowed");
                    return HasTargetGivenPushSourceOrNone(action.ActionContext.Hub, action.TargetSiteCollection)
                        ? VerificationRuleResult.Success
                        : VerificationRuleResult.Failed($"Site {action.TargetSiteCollection} is already receiving from another Content Type Deployment Hub. A Site Collection cannot receive from more than one Hub");

                }
                catch (Exception ex)
                {
                    Logger.Error("Error getting webproperty in HubAllowedRule", ex);
                    return VerificationRuleResult.Failed($"Site {action.TargetSiteCollection} is not accessible.");
                }
            }
        }
